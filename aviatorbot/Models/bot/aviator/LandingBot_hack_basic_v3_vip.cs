...
{
    public class LandingBot_hack_basic_v3_vip : AviatorBotBase
    {
        #region vars
        ...
        public LandingBot_hack_basic_v3_vip(BotModel model, IOperatorStorage operatorStorage, IBotStorage botStorage, ILogger logger) : base(model, operatorStorage, botStorage, logger)
        {
            Geotag = model.geotag;
            Token = model.token;
            Link = model.link;
        ...
        public override async Task<bool> Push(long id, string code, string uuid, int notification_id, string? firstname)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    var tmp = MessageProcessor.GetPush(statusResponse, code, link: Link, support_pm: SUPPORT_PM, pm: PM, isnegative: false, vip: Vip, help: Help);
